...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def timesAggregator(chars: List[Char], aggregator: List[(Char, Int)]): List[(Char, Int)] = {
      if(chars.size == 0) aggregator
      else {
        val count: Int = aggregator.find(p => chars.head == p._1).getOrElse(Nil,0)._2
        val newAggregator = aggregator.filterNot(p => p._1 == chars.head)
        timesAggregator(chars.tail, (chars.head, count+1) :: newAggregator) 
      }
    }
    timesAggregator(chars, List())
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
	  val sortedFreqs = freqs.sortWith((l,r) => l._2 < r._2)
	  sortedFreqs.map(pair => new Leaf(pair._1, pair._2))
  }
   ...
   */
  def until(hasReachedEnd: List[CodeTree] => Boolean, combinationAlgorithm: List[CodeTree]=> List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if(hasReachedEnd(trees)) trees
    else until(hasReachedEnd, combinationAlgorithm)(combinationAlgorithm(trees))
  }
   ...
   */
  def convert(tree: CodeTree): CodeTable = convertCurrent(tree, List())
  
  def convertCurrent(tree: CodeTree, encodingTillNow:List[Bit]): CodeTable = tree match{
    case Leaf(char, _) => List((char, encodingTillNow))
    case Fork(left, right,_,_) => convertCurrent(left, encodingTillNow :+ 0) ++ convertCurrent(right, encodingTillNow :+ 1)
  } 
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    (text.flatMap(character => codeTable.find(entry => entry._1 == character).get._2))
  }
}
